...
Revision ID: b44da7d412a6
Revises:
Create Date: 2025-07-16 13:24:33.704276
...
def upgrade() -> None:
    ...
    sa.PrimaryKeyConstraint('auth0_id', name=op.f('pk_auth0role')),
    sa.UniqueConstraint('auth0_id', name=op.f('uq_auth0role_auth0_id'))
    )
    ...
    sa.ForeignKeyConstraint(['role_id'], ['auth0role.auth0_id'], name=op.f('fk_grouprolelink_role_id_auth0role')),
    sa.PrimaryKeyConstraint('group_id', 'role_id', name=op.f('pk_grouprolelink'))
    )
    # ### end Alembic commands ###
    # NOTE: alembic doesn't seem to autogenerate these unique constraints (despite including them in the create_table commands)
    op.create_unique_constraint(op.f('uq_auth0role_auth0_id'), 'auth0role', ['auth0_id'])
...
def downgrade() -> None:
    ...
    op.drop_constraint(op.f('uq_auth0role_auth0_id'), 'auth0role', type_='unique')
    # ### commands auto generated by Alembic - please adjust! ###
