...
def test_approve_group_membership_invalid_role(test_client, test_db_session, galaxy_platform, persistent_factories, test_auth0_client):
    """
    Test approving groups returns an error when the admin doesn't have the required role
    """
    admin_role = Auth0RoleFactory.create_sync(name="biocommons/role/tsi/admin")
    group = BiocommonsGroupFactory.create_sync(group_id="biocommons/group/tsi", admin_roles=[admin_role])
    # User has admin rights to a platform but not this group
    access_token = AccessTokenPayloadFactory.build(biocommons_roles=["biocommons/role/galaxy/admin"])
    unauthorized_admin = SessionUserFactory.build(access_token=access_token)
    app.dependency_overrides[get_session_user] = lambda: unauthorized_admin
    user = Auth0UserDataFactory.build()
    BiocommonsUserFactory.create_sync(id=unauthorized_admin.access_token.sub, group_memberships=[], platform_memberships=[])
